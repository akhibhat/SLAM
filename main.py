...
def test_prediction(src_dir, dataset_id=0, split_name='train', log_dir='logs', is_online_fig=False):
    """ This function is created to debug your prediction step
        It runs the prediction step only
        After running this function, two figures are generated:
            One represents the trajectory generated using only lidar odometry information
            One represents the trajectory generated using only prediction step with every small process noise.
        You should expect that the two figures look very similar
        We set the motion covariance super small:
    ...
    lidar_odom_fig_path = log_dir + '/lidar_odom_only_traj_' + split_name + '_' + str(dataset_id) + '.jpg'
    ...
    prediction_fig_path = log_dir + '/prediction_only_traj_' + split_name + '_' + str(dataset_id) + '.jpg'
...
def test_update(src_dir, dataset_id=0, split_name='train', log_dir='logs', map_resolution=None, is_online_fig=False):
    """ This function is created to debug the update step.
    It creates 3 particles: np.array([[0.2, 2, 3], [0.4, 2, 5], [0.1, 2.7, 4]])
    * Note that the shape is 3 x num_p so the first particle is [0.2, 0.4, 0.1]
    It builds the first map and updates the 3 particles for 1 time step
    After running this function, you should expect that the weight of the first particle is largest since it is the closest to [0,0,0]
    ...
    # Generate 3 particles randomly to test update
    num_p = 3
    weights = np.ones(num_p) * 1.0/num_p
    ...
    print("----------------------------------")
    print("You should compare your printed results with that given in the above comment of the test_update func!")
...
def main():
    ...
    parser.add_argument('--running_mode', help="Test prediction/Update/SLAM?",    default='test_prediction', type=str,\
